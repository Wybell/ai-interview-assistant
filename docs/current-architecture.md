...
- 上传仅接受 PDF、DOCX、TXT，文件大小上限为 10 MB。
- 原始文件存入后端私有挂载目录，数据库保存元数据、存储路径和提取后的文本。
- 预览接口按当前用户和简历 ID 做所有权查询，只返回提取文本与文件元数据，不暴露存储路径或公开下载地址。
- 模拟面试会冻结会话创建时的有效模型，支持初轮技术面、深入技术面、综合终面、HR 沟通面及可选目标公司风格。
- 每次只生成一个主问题；每个主问题最多可追问两次。逐题保存回答、评分、参考要点和建议，正常结束或提前结束后均可生成 AI 复盘报告。
- 简历删除会保留已结束模拟面试的历史记录，仅清理私有文件与简历元数据；进行中的会话仍会阻止删除。

## 数据与发布

- MySQL 核心数据：用户、答题记录、AI 模型目录、知识库、简历、模拟面试会话及轮次。
- Redis：题目缓存与可重建的知识库题目历史。
- Flyway：V1 至 V13 均为版本化迁移。已在环境执行的迁移不可修改。
- Docker Compose：MySQL、Redis、Spring Boot 后端和 Vue/Nginx 前端均容器化运行。
- 宿主机 Nginx 负责公网入口和 TLS；容器端口绑定到 `127.0.0.1`，不直接暴露数据库或后端服务。
- GitHub Actions：每次推送 `main` 自动执行后端和前端 CI；生产发布仅允许人工确认后通过 SSH 在腾讯云执行。
